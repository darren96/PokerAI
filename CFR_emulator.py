...
class CFR:
    ...
    def simplify_hand(hand, community_cards):
        ...
        if key1 in holes:
            hole_group = holes[key1]
            hole = "_%dG" % holes[key1]
        elif key2 in holes:
            hole_group = holes[key2]
            hole = "_%dG" % holes[key2]
        ...
        if len(community_cards) != 0:
            ...
            if communities[hole_group].has_key(key):
                hand = "%s_%dG" % (hole, communities[hole_group].get(key))

        return hand


    def train(self, iterations, ante=1.0, bet1=2.0, bet2=8.0, print_interval=10, save_interval=1000):
